...
abstract class TweetSet {
   ...
   */
    def union(that: TweetSet): TweetSet = {
      def unionHelper(tweetSet: TweetSet, acc: TweetSet): TweetSet = {
        if (tweetSet.isEmpty) acc
        else unionHelper(tweetSet.tail, acc.incl(tweetSet.head))
      }

      unionHelper(that, this)
    }
    ...
    def mentions(keywords: List[String]): TweetSet = {
      filter(x => x.mentions(keywords))
    }
  /**
...
class Empty extends TweetSet {
  ...
  override def head: Tweet = throw new NoSuchElementException("Empty.head")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      val otherHit = right.filterAcc(p, left.filterAcc(p, acc))

      if (p(elem)) otherHit incl elem
      else otherHit
